...
function normalize(){for(const k of["health","happiness","smarts","looks","discipline","karma"])state[k]=clamp(state[k]);state.money=Math.max(0,Math.round(state.money));state.bank=Math.max(0,Math.round(state.bank));state.debt=Math.max(0,Math.round(state.debt))}
...
function interact(i){const r=state.relationships[i];r.closeness=clamp(r.closeness+rand(5,18));state.happiness=clamp(state.happiness+rand(2,7));log("Időt töltöttél "+r.name+" társaságában. Kapcsolat: "+r.closeness+"%.","Kapcsolat");render()}
function marryAction(){const r=state.relationships.find(x=>x.closeness>=70);if(!r)return toast("Nincs még elég erős kapcsolatod.");state.flags.married=true;r.type="Házastárs";state.happiness=clamp(state.happiness+15);log("Összeházasodtál "+r.name+"-nel!","Kapcsolat");checkAchievements();render()}
function childAction(){if(!state.flags.married)return toast("Előbb házasodj meg.");if(state.age<18||state.age>50)return toast("Ebben az életkorban ez most nem lehetséges.");const child={name:pick(names.female.concat(names.male))+" "+state.last,age:0};state.children.push(child);state.stats.children++;log("Megszületett "+child.name+"!","Család");checkAchievements();render()}
...
function pet(){if(state.assets.some(a=>a.name.includes("Háziállat")))return toast("Már van háziállatod.");buy("Háziállat","🐕",150000)}
...
$("startBtn").onclick=start;$("ageBtn").onclick=()=>state&&state.alive?nextYear():showDeath();$("saveBtn").onclick=save;$("newBtn").onclick=()=>{if(confirm("Biztosan új életet kezdesz?"))newLife()};$("modalClose").onclick=closeModal;$("tabs").addEventListener("click",e=>{const b=e.target.closest("button[data-tab]");if(b)showTab(b.dataset.tab)});if(load()){render();$("saveNotice").textContent="Mentett játék betöltve."}else render();
